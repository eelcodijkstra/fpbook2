...
\textbf{begrippen}: waarde, expressie, operator, operator-prioriteit (precedentie?), naam-definitie, naam-gebruik, rekenen/herschrijven (berekening)
...
\item functie-toepassing heeft in Haskell de vorm: \texttt{f 3}

\begin{itemize}
\item de naam van de functie met daarachter het argument waarop de functie toegepast wordt.
\item in plaats van \texttt{f(3)} (zoals gebruikelijk in veel andere programmeertalen, of in de wiskunde)
\item functie-toepassing bindt sterker dan operatoren als \texttt{+}, \texttt{*}, enz.
\end{itemize}
\end{itemize}
...
\textbf{begrippen}: functie-definitie, functie-naam, parameters, functie-toepassing (applicatie, aanroep), argumenten
...
voorbeelden: \texttt{sqr} (definitie en toepassing), \texttt{succ} (en functies met twee parameters?)
